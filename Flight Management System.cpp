...
const string l_destination[5]={"BANGLORE","HYDERABAD","MUMBAI","KOCHI","CHENNAI"};	
const string i_destination[11]={"LONDON","BERLIN","DUBAI","RIYADH","BEIJING","PARIS","CANBERRA","TOKYO","WASHINGTON","DELHI","ISLAMABAD" }; 
...
class booking:public passenger
{
	...
	bool ldest()
	{
		...
		else
		{
			for(i=0;i<5;i++)
			{
				if(l_destination[i] == origin)
					n++;
			}
			for(j=0;j<5;j++)
			{
				if(l_destination[j] == destination)
					m++;
			}
			if(m==1&&n==1)
			{
		...
		bool idest(string pass)
	{
		...
		else
		{
			for(i=0;i<11;i++)
			{
				if(i_destination[i] == origin)
					n++;
			}
			for(j=0;j<11;j++)
			{
				if(i_destination[j] == destination)
					m++;
			}
			if(m==1&&n==1)
			{
...
class about
{
	public:
		void Aboutus()
		{
			cout<<"Our airlines was founded in 1989 ,and is one of the first airlines to offer international.\nWith our many years of experience we Have earned a platinum certification from Airline Magazine.\nWe have added multiple menu options from onboard food.\nTravel in style with DTU Airlines."<<endl<<endl;
...
int main()
{
	cout<<"\n\t\t\t\tObject Orineted Programming Project\n\t\t  FLIGHT MANAGEMENT SYSTEM";
